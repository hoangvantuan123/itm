  ...
  async getPersonnelInterById(id: number): Promise<any> {
    ...
    return {
      ...
      data: {
        ...
        interviewer_user: inter.interviewer_user,
        interview_results: inter.interview_results,
  ...
  async synchronizeHr(ids: number[]): Promise<void> {
    try {
      ...
      const hrErpRecords = personnels.map((personnel) => ({
        ...
        note: personnel.p_note,
        interview_results: personnel.p_interview_results,
        interviewer_user: personnel.p_interviewer_user
      }));
